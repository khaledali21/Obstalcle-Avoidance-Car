...
LED_ERR_STATE LED_u8Init(const LED_t* led){
	...
	if(led == NULL){
		...
		state = DIO_u8SetPinDirection(led->PORT, led->PIN, DIO_OUTPUT);
	}
	return state;
}
/*
...
LED_ERR_STATE LED_u8ON(const LED_t* led){
	...
	if(led == NULL){
		...
		state = DIO_u8SetPinData(led->PORT, led->PIN, DIO_HIGH);
	}
	return state;
}
/*
...
LED_ERR_STATE LED_u8OFF(const LED_t* led){
	...
	if(led == NULL){
		...
		state = DIO_u8SetPinData(led->PORT, led->PIN, DIO_LOW);
	}
	return state;
}
/*
